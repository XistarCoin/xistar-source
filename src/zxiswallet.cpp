...
CzXISWallet::CzXISWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zxis seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZXISSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZXISSeed_deprecated()) {
                    LogPrintf("%s: Updated zXIS seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zxis seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zxis wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzXISWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZXISCount(nCountLastUsed);
    else if (!walletdb.ReadZXISCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzXISWallet::Lock()
{
    seedMaster = 0;
}

void CzXISWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzXISWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZXIS(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzXISWallet::LoadMintPoolFromDB()
{
...
void CzXISWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzXISWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzXISWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zxisTracker->HasPubcoinHash(pMint.first)) {
...
bool CzXISWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZXIS(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zxisTracker which also adds to database
    pwalletMain->zxisTracker->Add(dMint, true);
...
void CzXISWallet::SeedToZXIS(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzXISWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzXISWallet::GenerateDeterministicZXIS(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzXISWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzXISWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
