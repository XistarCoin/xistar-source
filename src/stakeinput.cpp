...
// The zXIS block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zXIS is a hash of the serial
...
{
    //Create an output returning the zXIS that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZXISOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zXIS output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zXIS", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZXISOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zXIS output", __func__);
...
{
    CzXISTracker* zxisTracker = pwallet->zxisTracker.get();
    CMintMeta meta;
    if (!zxisTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zxisTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a XIS stake is the outpoint
