...
public:
    ...
    bool WriteZXISSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZXISSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZXISSeed_deprecated(uint256& seed);
    bool EraseZXISSeed();
    bool EraseZXISSeed_deprecated();

    bool WriteZXISCount(const uint32_t& nCount);
    bool ReadZXISCount(uint32_t& nCount);
