...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::XIS);
    ...
    if (!settings.contains("nAnonymizeXistarAmount"))
        settings.setValue("nAnonymizeXistarAmount", 1000);

    nAnonymizeXistarAmount = settings.value("nAnonymizeXistarAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeXistarAmount"))
        SoftSetArg("-anonymizexistaramount", settings.value("nAnonymizeXistarAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in xistar.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeXistarAmount:
            return QVariant(nAnonymizeXistarAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeXistarAmount:
            nAnonymizeXistarAmount = value.toInt();
            settings.setValue("nAnonymizeXistarAmount", nAnonymizeXistarAmount);
            emit anonymizeXistarAmountChanged(nAnonymizeXistarAmount);
            break;
        case CoinControlFeatures:
