...
#ifndef Xistar_ZXISWALLET_H
#define Xistar_ZXISWALLET_H
...
public:
    CzXISWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZXIS(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZXIS(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //Xistar_ZXISWALLET_H
